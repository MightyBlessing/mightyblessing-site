---
title: WELOVE 공연 운영 케이스
slug: welove-case
date: '2023-06-01'
featured: true
featured_order: 1
location: '부산, 전주, 용인'
summary: 'WELOVE TOUR 2022 "다시, Back to the Garden" 등 대형 공연의 기획·운영·현장 오퍼레이션을 담당했습니다.'
roles:
  - 기획
  - 운영
  - 현장
  - 무대
categories:
  - 예배·집회
  - 투어·라이브
search_terms:
  - WELOVE
  - welove tour
  - Back to the Garden
  - 부산 공연
  - 전주 공연
  - 용인 공연
heroMedia:
  type: video
  storageKey: portfolio/welove-case/hero.mp4
  posterStorageKey: portfolio/welove-case/poster.jpg
  alt: WELOVE 공연 현장의 열기와 조명을 담은 라이브 영상
goals: |
  대규모 찬양 공연의 원활한 진행과 참가자 경험 극대화.
  지역별 투어 일정·인력·장비를 체계적으로 관리.
our_role: |
  - 전체 기획 및 일정·예산 관리
  - 스탭 모집·교육·배치
  - 무대 연출·소품·조명·음향 협업
  - 접수·숙소·현장 운영
process: |
  1. 준비: 컨셉 회의, 일정 수립, 파트너사 협의, 체크리스트 작성
  2. 현장: 리허설, 스태프 브리핑, 실시간 대응
  3. 사후: 회고, 자료 정리, 피드백 반영
metrics:
  - label: 투어 지역
    value: 3개 도시
  - label: 참가 규모
    value: 대형
testimonials: []
gallery:
  - type: image
    storageKey: portfolio/welove-case/detail-01.jpg
    alt: 핑크 조명으로 구성된 공연 무대
    caption: 무대 셋업
  - type: image
    storageKey: portfolio/welove-case/detail-02.jpg
    alt: 무대를 가로지르는 청록빛 조명
    caption: 조명 디테일
  - type: image
    storageKey: portfolio/welove-case/detail-03.jpg
    alt: 찬양과 예배에 집중하는 참가자들
    caption: 현장 반응
  - type: image
    storageKey: portfolio/shared/crowd-arms.jpg
    alt: 공연 현장에서 함께 손을 들고 반응하는 참가자들
    caption: 군중의 열기
related_cases:
  - the-sent-case
  - regional-worship-case
mediaId: welove-case
---
WELOVE 공연 시리즈의 투어 및 단일 공연 운영을 맡았습니다. 기독교 청년들을 위한 대형 찬양·예배 행사로, 기획 단계부터 현장 당일까지 일관된 운영 시스템을 적용했습니다.

파트너 팀과의 역할 분담, 지역별 장소·인프라 협의, 참가자 접수·안내까지 하나의 플로우로 설계해 재사용 가능한 매뉴얼을 정리했습니다.
